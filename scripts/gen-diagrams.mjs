...
const DIAGRAMS = [
  {
    ...
    title: "Tiered Multimodal VLM Evaluation (production)",
    subtitle: "golden-set policy · cost-aware routing · human-reviewed output",
    rows: [{ boxes: [
      { title:"MTBF evidence", lines:["frames + telemetry","inspection context"], accent:"cyan" },
      { title:"Rule tier", lines:["deterministic 79.5%","of 1,110 cases"], accent:"violet" },
      { title:"Local VLM", lines:["default evaluator","self-consistency"], accent:"green", emphasis:true },
      { title:"Bedrock · Claude Opus", lines:["ambiguous ~16%","transfer approval"], accent:"amber" },
      { title:"Review gate", lines:["human-approved only","to final KPI"], accent:"cyan" },
    ]}],
    legend: [
      "evolved from Gemini on Vertex AI",
      "OpenAI-compatible local VLM",
      "schema-constrained output",
      "selective Bedrock escalation + cost controls",
      "unreviewed AI excluded from final KPI",
    ],
  },
  {
  ...
  // ---- platform & SVEvalFlow project diagrams ----
  {
    ...
    rows: [
      ...
      { label:"KUBERNETES CLUSTERS + STORAGE", flow:false, boxes:[
        { title:"On-prem RKE2", lines:["60+ heterogeneous GPU"], accent:"amber" },
        { title:"NCP NKS", lines:["L40S GPU workers"], accent:"amber" },
        { title:"Autoscaling", lines:["1 → 14 → 1 nodes"], accent:"amber" },
        { title:"Object stores", lines:["HCP / NCP"], accent:"amber" },
      ]},
    ],
    legend: [
      "led scheduler & queue policy design",
      "Scale-to-Zero serverless inference",
      "deficit-based urgent GPU preemption + auto recovery",
  ...
  },
  {
    ...
    rows: [{ boxes: [
      ...
      { title:"Artifact contract", lines:["cu118 / cu128","version · digest"], accent:"amber" },
    ...
    legend: [
      ...
      "MTBF lookup/build API by branch · config · commit",
    ],
  },
  {
    file: "prism.svg",
  ...
  },
  {
    ...
    rows: [{ boxes: [
      ...
      { title:"GPU pods", lines:["RKE2 / NKS L40S","autoscale + fan-out"], accent:"green", emphasis:true },
      { title:"AEB evaluation", lines:["9 configs","3 TTC × 3 frames"], accent:"amber" },
      { title:"Tiered VLM", lines:["rules → local","→ Bedrock"], accent:"cyan" },
      { title:"Review + KPI", lines:["human-approved","Postgres · dashboard"], accent:"cyan" },
    ]}],
    legend: [
      "430,851 clips · 99.87% cumulative",
      "NCP: 57,745 commands / 1,000h · 99.94%",
      "GPU nodes autoscaled 1 → 14 → 1",
      "rules → local VLM → selective Bedrock escalation",
    ],
  },
];
